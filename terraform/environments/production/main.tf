...
module "elasticache" {
  ...
  preferred_azs         = ["${var.aws_region}a", "${var.aws_region}b"]
...
module "eks" {
  ...
  node_groups = {
    reserved = {
      instance_types = [var.eks_reserved_instance_type]
      capacity_type  = "ON_DEMAND"
      desired_size   = var.eks_reserved_desired
      min_size       = var.eks_reserved_min
      max_size       = var.eks_reserved_max
      labels         = { role = "app", capacity = "reserved", env = local.env }
    }
    ondemand-burst = {
      instance_types = [var.eks_burst_instance_type]
      capacity_type  = "ON_DEMAND"
      desired_size   = var.eks_burst_desired
      min_size       = var.eks_burst_min
      max_size       = var.eks_burst_max
      labels         = { role = "app", capacity = "burst", env = local.env }
    }
  }
}
